...
const ProductBacklog = () => {
    ...
    return (
        <div className="App">
            ...
            <div className="tab">
                <button
                    className={`tablink ${activeTab === 'Unfinished' ? 'active' : ''}`}
                    onClick={() => handleTabClick('Unfinished')}
                >
                    Unfinished stories
                </button>
                <button
                    className={`tablink ${activeTab === 'Finished' ? 'active' : ''}`}
                    onClick={() => handleTabClick('Finished')}
                >
                    Finished user stories
                </button>
                <button
                    className={`tablink ${activeTab === 'FutureReleases' ? 'active' : ''}`}
                    onClick={() => handleTabClick('FutureReleases')}
                >
                    Future releases
                </button>
            </div>
            ...
            <div className="tabcontent">
                {activeTab === 'Unfinished' && <Unfinished addPost={addPost} posts={posts}/>}
                {activeTab === 'Finished' && <Finished/>}
                {activeTab === 'FutureReleases' && <FutureReleases/>}
            </div>
        </div>
    );
};
const Unfinished = ({addPost, posts}) => {
...
const FutureReleases = ({addPost, posts}) => {
    const [message, setMessage] = useState('');
    const [messages, setMessages] = useState([]);
    const [currentUser, setCurrentUser] = useState({});
    const [commentTexts, setCommentTexts] = useState({});
